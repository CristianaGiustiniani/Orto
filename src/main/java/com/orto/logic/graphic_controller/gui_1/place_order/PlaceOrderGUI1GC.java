...
public class PlaceOrderGUI1GC extends GUIGC implements PlaceOrderGC {
    ...
    //CONSTRUCTOR
    public PlaceOrderGUI1GC(SellerBean seller) {
    ...
    public void collectData() throws NotPositiveQuantityException, AnnotationTooLongException, WrongFormatQuantityException, InvalidDeliveryInfoException, NoProductSelectedException, FailedPaymentException {
        switch (currentStep) {
            case PRODUCT_SELECTION:
                List<OrderLineBean> lines = this.productSelectionGUI1GC.getLines();
                controller.addProductsToOrder(lines);
                break;
            case DELIVERY_SELECTION:
                DeliveryBean delivery = this.deliverySelectionGUI1GC.getDeliveryInfo();
                controller.defineDelivery(delivery);
                break;
            case PAYMENT_SELECTION:
                PaymentBean payment = this.paymentSelectionGUI1GC.getPaymentInfo();
